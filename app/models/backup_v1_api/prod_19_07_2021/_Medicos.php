...
//..
    ...
    public function getDirectorioMedicos_Rand()
    {
        ...
        try {
            ...
            if ($this->searchField != null) {
            ...
            } elseif ($this->sortCategory != null && $this->searchType == null) {

                $this->sortCategory = mb_strtoupper($this->sortCategory);

                #setear valores para alergologia
                if ($this->sortCategory == 'ALERGOLOGIA') {
                ...
                } elseif ($this->sortCategory == 'ODONTOLOGIA') {

                    # Devolver valores por especialidad de medicos
                    $sql = " SELECT WEB_VW_DATOS_MEDICOS.*, ROWNUM AS ROWNUM_ FROM WEB_VW_DATOS_MEDICOS WHERE COD_MEDICO
                    IN ('010349','678','347')  ORDER BY APELLIDOS ASC ";

                } elseif ($this->sortCategory == 'ENDOCRINOLOGIA') {

                    # Devolver valores por especialidad de medicos
                    $sql = " SELECT WEB_VW_DATOS_MEDICOS.*, ROWNUM AS ROWNUM_ FROM WEB_VW_DATOS_MEDICOS WHERE COD_MEDICO
                    IN ('546','674','226','343','360','811')  ORDER BY APELLIDOS ASC ";

                } elseif ($this->sortCategory == 'ACARDIOLOGIA') {

                    # Devolver valores por especialidad de medicos
                    $sql = " SELECT WEB_VW_DATOS_MEDICOS.*, ROWNUM AS ROWNUM_ FROM WEB_VW_DATOS_MEDICOS WHERE COD_MEDICO
                    IN ('010452','30','10764','10791','294','010792','522','549','010454','010079','643')  ORDER BY APELLIDOS ASC ";

                } elseif ($this->sortCategory == 'CARDIOLOGIA PEDIATRICA') {

                    # Devolver valores por especialidad de medicos
                    $sql = " SELECT WEB_VW_DATOS_MEDICOS.*, ROWNUM AS ROWNUM_ FROM WEB_VW_DATOS_MEDICOS WHERE COD_MEDICO
                    IN ('126','86','456')  ORDER BY APELLIDOS ASC ";

                } elseif ($this->sortCategory == 'CUIDADOS INTENSIVOS PEDIATRICOS') {

                    # Devolver valores por especialidad de medicos
                    $sql = " SELECT WEB_VW_DATOS_MEDICOS.*, ROWNUM AS ROWNUM_ FROM WEB_VW_DATOS_MEDICOS WHERE COD_MEDICO
                    IN ('010209','809')  ORDER BY APELLIDOS ASC ";

                } elseif ($this->sortCategory == 'ODONTOPEDIATRIA') {

                    # Devolver valores por especialidad de medicos
                    $sql = " SELECT WEB_VW_DATOS_MEDICOS.*, ROWNUM AS ROWNUM_ FROM WEB_VW_DATOS_MEDICOS WHERE COD_MEDICO
                    IN ('347')  ORDER BY APELLIDOS ASC ";

                } elseif ($this->sortCategory == 'NEUROLOGIA PEDIATRICA') {

                    # Devolver valores por especialidad de medicos
                    $sql = " SELECT WEB_VW_DATOS_MEDICOS.*, ROWNUM AS ROWNUM_ FROM WEB_VW_DATOS_MEDICOS WHERE COD_MEDICO
                    IN ('12', '010481', '10608', '73', '476', '010365')  ORDER BY APELLIDOS ASC ";
                ...
                // Cambio solicitado por Caridad 06/Nov/2020
                ...
                if ($this->searchType) {
                    ...
                    # Devolver valores por especialidad de medicos
                    $sql = "SELECT WEB_VW_DATOS_MEDICOS.*, ROWNUM AS ROWNUM FROM WEB_VW_DATOS_MEDICOS WHERE ESPECIALIDAD = '$this->sortCategory' AND CATEGORIA='ACTIVO' ORDER BY APELLIDOS ASC ";
