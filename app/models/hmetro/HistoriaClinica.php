...
use app\models\hmetro as Model;
...
class HistoriaClinica extends Models implements IModels
{
    ...
    /**
     * Inserta pago de flow reliv // actualiza_fact_medico
     */
    public function actualiza_fact_medico()
    {
        //Inicialización de variables
        global $http;
        ...
        try {
            ...
            $pc_id_medico = $http->request->get('pc_id_medico');
            $pc_ruc_medico = $http->request->get('pc_ruc_medico');
            $pn_hcl = $http->request->get('pn_hcl');
            $pn_adm = $http->request->get('pn_adm');
            $pc_prefactura = $http->request->get('pc_prefactura');
            $pc_factura_origen = $http->request->get('pc_factura_origen');
            $pc_serie_factura_med = $http->request->get('pc_serie_factura_med');
            $pc_numer_factura_med = $http->request->get('pc_numer_factura_med');
            $pc_clave_acceso = $http->request->get('pc_clave_acceso');

            if ($pc_id_medico == null) {
                throw new Exception('Todos los parámetros son obligatorios. PARAM: [pc_id_medico]');
            }

            if ($pc_ruc_medico == null) {
                throw new Exception('Todos los parámetros son obligatorios. PARAM: [pc_ruc_medico]');
            }

            if ($pn_hcl == null) {
                throw new Exception('Todos los parámetros son obligatorios. PARAM: [pn_hcl]');
            }

            if ($pn_adm == null) {
                throw new Exception('Todos los parámetros son obligatorios. PARAM: [pn_adm]');
            }

            if ($pc_prefactura == null) {
                throw new Exception('Todos los parámetros son obligatorios. PARAM: [pc_prefactura]');
            }

            if ($pc_factura_origen == null) {
                throw new Exception('Todos los parámetros son obligatorios. PARAM: [pc_factura_origen]');
            }

            if ($pc_serie_factura_med == null) {
                throw new Exception('Todos los parámetros son obligatorios. PARAM: [pc_serie_factura_med]');
            }

            if ($pc_numer_factura_med == null) {
                throw new Exception('Todos los parámetros son obligatorios. PARAM: [pc_numer_factura_med]');
            }

            if ($pc_clave_acceso == null) {
                throw new Exception('Todos los parámetros son obligatorios. PARAM: [pc_clave_acceso]');
            }

            //Conectar a la BDD
            $this->conexion->conectarDesa();
            ...
            $stid = oci_parse($conexion, "BEGIN PRO_ACT_FACT_MEDICO(:pc_id_medico, :pc_ruc_medico, :pn_hcl, :pn_adm, :pc_prefactura, :pc_factura_origen, :pc_serie_factura_med, :pc_numer_factura_med, :pc_clave_acceso, :pc_error); END;");

            // Bind the input num_entries argument to the $max_entries PHP variable
            oci_bind_by_name($stid, ":pc_id_medico", $pc_id_medico, 33);
            oci_bind_by_name($stid, ":pc_ruc_medico", $pc_ruc_medico, 33);
            oci_bind_by_name($stid, ":pn_hcl", $pn_hcl, 10);
            oci_bind_by_name($stid, ":pn_adm", $pn_adm, 10);
            oci_bind_by_name($stid, ":pc_prefactura", $pc_prefactura, 33);
            oci_bind_by_name($stid, ":pc_factura_origen", $pc_factura_origen, 33);
            oci_bind_by_name($stid, ":pc_serie_factura_med", $pc_serie_factura_med, 33);
            oci_bind_by_name($stid, ":pc_numer_factura_med", $pc_numer_factura_med, 33);
            oci_bind_by_name($stid, ":pc_clave_acceso", $pc_clave_acceso, 66);

            // Bind the output parameter
            oci_bind_by_name($stid, ':pc_error', $mensajeError, 120);

            //Ejecuta el SP
            $r = oci_execute($stid, OCI_DEFAULT);

            if ($r == true && $mensajeError == 'N') {

                return array(
                    'status' => true,
                    'message' => 'Procesado realizado con éxito. ',
                );

            } else {

                $e = oci_error($stid);
                $mensajeError = "Error en SP, consulte con el Administrador del Sistema. " . $e['message'];
