...
//..
    ...
    public function contactChequeoMedico(array $data): array
    {
        ...
        $datos = array(
            ...
            // Nombres
            "entry.1250398818" => strtoupper($data['nombre']),
            // Apellidos
            "entry.2095219822" => strtoupper($data['ape']),
            // Cédula
            "entry.1175959311" => strtoupper($data['ced']),
            // Email
            "entry.1226073019" => strtoupper($data['email']),
            // Celular
            "entry.725180973"  => strtoupper($data['cel']),
            // Ciudad
            "entry.720586897"  => (isset($data['ciudad'])) ? strtoupper($data['ciudad']) : '',
            // Comment
            "entry.1861082397" => (isset($data['comment'])) ? strtoupper($data['comment']) : '',
            // Tipo de Chequeo
            "entry.1463226498" => strtoupper($data['tipo-chequeo']),
            // Canl
            "entry.846775110"  => (isset($data['target'])) ? strtoupper($data['target']) : 'WEB',

        );
        //url contra la que atacamos
        $ch = curl_init("https://docs.google.com/forms/d/e/1FAIpQLSfhcwxNiggehUtk3IUlE0f5EmjzQs9lbnaDsKVDMG_zoVpGIQ/formResponse");
        //a true, obtendremos una respuesta de la url, en otro caso,
        //true si es correcto, false si no lo es
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        //establecemos el verbo http que queremos utilizar para la petición
