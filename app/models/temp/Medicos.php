    ...
    public function getPerfil_Medico_Buscador($COD_MEDICO): array
    {
        ...
        try {
            ...
            $queryBuilder
                ...
                ->setParameter('COD_MEDICO', $COD_MEDICO)
            ;
            ...
            $foto_medico = file_exists('assets/doctores/' . $medico['COD_MEDICO'] . '.jpg');

            # OBTENER IMAGEN FOTO DE MEDICO
            $medico['FOTO'] = ($foto_medico) ? $config['api']['url'] . 'assets/doctores/' . $medico['COD_MEDICO'] . '.jpg' : false;
    ...
    public function getDirectorioMedicos_Rand()
    {
        ...
        try {
            ...
            if ($this->searchField != null) {
                ...
                } elseif (mb_strtoupper($this->searchField) == 'ODONTOPEDIATRIA') {

                    # Devolver valores por especialidad de medicos
                    $sql = " SELECT WEB_VW_DATOS_MEDICOS.*, ROWNUM AS ROWNUM_ FROM WEB_VW_DATOS_MEDICOS WHERE COD_MEDICO
                    IN ('347')  ORDER BY APELLIDOS ASC ";

                } elseif (mb_strtoupper($this->searchField) == 'CARDIOLOGIA') {

                    # Devolver valores por especialidad de medicos
                    $sql = " SELECT WEB_VW_DATOS_MEDICOS.*, ROWNUM AS ROWNUM_ FROM WEB_VW_DATOS_MEDICOS WHERE COD_MEDICO
                    IN ('010452','30','10764','10791','294','10792','522','549','10454','10079','643')  ORDER BY APELLIDOS ASC ";

                } elseif (mb_strtoupper($this->searchField) == 'ELECTROFISIOLOGIA') {

                    # Devolver valores por especialidad de medicos
                    $sql = " SELECT WEB_VW_DATOS_MEDICOS.*, ROWNUM AS ROWNUM_ FROM WEB_VW_DATOS_MEDICOS WHERE COD_MEDICO
                    IN ('10792','643')  ORDER BY APELLIDOS ASC ";

                } elseif (mb_strtoupper($this->searchField) == 'HEMODINAMICA') {

                    # Devolver valores por especialidad de medicos
                    $sql = " SELECT WEB_VW_DATOS_MEDICOS.*, ROWNUM AS ROWNUM_ FROM WEB_VW_DATOS_MEDICOS WHERE COD_MEDICO
                    IN ('30','10791','522')  ORDER BY APELLIDOS ASC ";

                } elseif (mb_strtoupper($this->searchField) == 'ONCOLOGIA') {
                ...
                #setear valores para alergologia
                if (mb_strtoupper($this->sortCategory) == 'ALERGOLOGIA') {

                    # Devolver valores por especialidad de medicos
                    $sql = " SELECT WEB_VW_DATOS_MEDICOS.*, ROWNUM AS ROWNUM_ FROM WEB_VW_DATOS_MEDICOS WHERE COD_MEDICO IN ('215','252')  ORDER BY APELLIDOS ASC ";

                } elseif (mb_strtoupper($this->sortCategory) == 'ODONTOLOGIA') {

                    # Devolver valores por especialidad de medicos
                    $sql = " SELECT WEB_VW_DATOS_MEDICOS.*, ROWNUM AS ROWNUM_ FROM WEB_VW_DATOS_MEDICOS WHERE COD_MEDICO
                    IN ('010349','678','347')  ORDER BY APELLIDOS ASC ";

                } elseif (mb_strtoupper($this->sortCategory) == 'ENDOCRINOLOGIA') {

                    # Devolver valores por especialidad de medicos
                    $sql = " SELECT WEB_VW_DATOS_MEDICOS.*, ROWNUM AS ROWNUM_ FROM WEB_VW_DATOS_MEDICOS WHERE COD_MEDICO
                    IN ('546','674','226','343','360','811')  ORDER BY APELLIDOS ASC ";

                } elseif (mb_strtoupper($this->sortCategory) == 'CARDIOLOGIA') {
            ...
            foreach ($medicos as $key) {
                if ($key['FOTO'] != false || $key['COD_MEDICO'] == '10720') {
                    $_medicos[] = $key;
                }
            }
